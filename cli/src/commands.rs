...
impl CommandHandler {
    ...
    pub async fn start_background(&self) -> Result<JoinHandle<()>> {
        // Note: Client.start() doesn't actually work well in Arc<RwLock> pattern
        // since it needs mutable access and blocks. For now, just return a dummy handle.
        // The real fix would be to refactor Client to have its network run separately.
        let handle = tokio::spawn(async move {
            // Placeholder - Client.start() would need to be refactored
